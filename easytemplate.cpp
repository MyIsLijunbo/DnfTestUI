...
bool EasyTemplate::Match(cv::Mat &Img, cv::Rect &roi, float &score)
{
    ...
    cv::Point minLocation,maxLocation,matchLoc;
...
//    cv::imshow("match-result", Img);
    ...
    //计算直方图并且归一化
    cv::Mat images =  ScoreImg(roi).clone();
    cv::calcHist(&images, 1, channels, cv::Mat(), hist2, 1, bins, ranges);
...
bool EasyTemplate::Match(cv::Mat & Img, cv::RotatedRect & roi, float & score)
{
    ...
    UpdateImgAngle(gTemplate, gTemplateMask, Angle_Max, nTemplate, nTemplateMask,rect);
    ...
    cv::normalize(hist1, hist1, 1, 0, cv::NORM_L1);
    //计算直方图并且归一化
    cv::Mat images = ScoreImg(matchedROI).clone();
    cv::calcHist(&images, 1, channels, cv::Mat(), hist2, 1, bins, ranges);
    cv::normalize(hist2, hist2, 1, 0, cv::NORM_L1);
    //匹配计算相似度
    float similarity = cv::compareHist(hist1, hist2, cv::HISTCMP_INTERSECT);
